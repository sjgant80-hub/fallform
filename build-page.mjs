// build-page.mjs — put the GATED formrules guard inside the page.
//
// fallslot is a single offline file, so the kernel is inlined rather than imported — and inlined
// VERBATIM, or the code that was gated and the code that refuses a double formrules would be two
// different programs.
import { readFileSync, writeFileSync } from 'node:fs';

const OPEN = '/* __FORMRULES_KERNEL__ */';
const CLOSE = '/* __END_FORMRULES_KERNEL__ */';

// Only the export KEYWORD goes. The \r? matters: these files are CRLF and `.` stops at \r.
const kernel = readFileSync('formrules.mjs', 'utf8')
  .replace(/^#!.*\r?\n/, '')
  .replace(/^import[^\n]*\n/gm, '')
...
for (const fn of ['function isAnswered', 'function validate', 'function accept']) {
...
console.log(`index.html — formrules guard inlined, ${kernel.split('\n').length} lines, page ${(out.length / 1024).toFixed(0)}KB`);
